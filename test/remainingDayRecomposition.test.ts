...
async function main() {
  ...
  check('87/88/91. no migration, dependency or Home wiring: 39 migrations, no Home/Timeline/Daily Agenda file mentions recomposition', fs.readdirSync(path.join(root, 'apps/web/prisma/migrations')).filter((d) => /^\d{4}_/.test(d)).length === 39 && ['apps/web/components/HomeTimeline.tsx', 'apps/web/lib/dailyAgenda.ts', 'apps/web/lib/homeTimelineComposer.ts', 'apps/web/lib/homeCompletion.ts'].every((f) => !/recompos/i.test(read(f))) && !/remainingDayRecomposition/.test(read('apps/web/lib/planMove.ts')));
