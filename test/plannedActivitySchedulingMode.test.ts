...
check('50. (F1 scope, updated for F2) no batch-move / recomposition-ACCEPTANCE module or route exists -- the read-only proposal service (F2) is the only recomposition code, and F1 mode semantics are consumed only through hasFlexibleScheduling', !prod.some((f) => /(batchMove|applyRecomposition|recompositionAccept|recomposeAccept)/i.test(f)) && !fs.existsSync(path.join(root, 'apps/web/app/api/day/recompose/accept')) && !fs.existsSync(path.join(root, 'apps/web/lib/recompositionAcceptance.ts')));
