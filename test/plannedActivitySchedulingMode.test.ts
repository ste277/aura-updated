...
check('50. no recomposition / batch-move / proposal module exists', !fs.existsSync(path.join(root, 'apps/web/lib/remainingDayRecomposition.ts')) && !prod.some((f) => /recompos/i.test(f)) && !fs.existsSync(path.join(root, 'apps/web/app/api/day/recompose')));
