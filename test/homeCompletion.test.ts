...
async function main() {
  ...
  check('4/29. Done is a real button (SecondaryButton) with an accessible name and disabled-while-saving semantics, only when a committed plan is completable', /completablePlanIdNow && \(\s*<SecondaryButton/.test(dash) && /ariaLabel=\{`Mark "\$\{spotlightItem\.title\}" done`\}/.test(dash) && /disabled=\{completingPlanIds\.has\(completablePlanIdNow\) \|\| skippingPlanIds\.has\(completablePlanIdNow\) \|\| movingPlanIds\.has\(completablePlanIdNow\)( \|\| recompositionAccepting)?\}/.test(dash));
  check('29. the whole Right Now card is NOT a completion control: exactly two call sites -- the Right Now Done button and the Missed Recovery Done button (PR E) -- never a card/row click', (dash.match(/handleCompleteRightNow\(/g) ?? []).length === 2 && /void handleCompleteRightNow\(planId, 'TIMELINE'\)/.test(dash) && /<SecondaryButton\s+onClick=\{\(\) => void handleCompleteRightNow\(completablePlanIdNow\)\}/.test(dash));
  check('31. copy is "Done" / "Saving…"; no HabitLog/LOGGED/PlannedActivity terminology in Home UI text', /completingPlanIds\.has\(completablePlanIdNow\) \? 'Saving…' : 'Done'/.test(dash) && !/Couldn't mark[^"]*(HabitLog|LOGGED|PlannedActivity)/.test(dash));
  check('7. no parallel completion API: the only endpoint referenced is /api/plans/<id>/log; no new route files', /\/api\/plans\/\$\{encodeURIComponent\(planId\)\}\/\$\{action\}/.test(helper) && /complete: \(planId: string\): Promise<CompletePlanResult> => run\(planId, 'log', 'LOGGED', 'DONE'\)/.test(helper) && !fs.existsSync(path.join(__dirname, '../apps/web/app/api/home')) && !fs.existsSync(path.join(__dirname, '../apps/web/app/api/right-now')));
  ...
  check('11/15/24. after a CONFIRMED success Home overlays only the confirmed plan id, then runs Home\'s own quiet refresh (page.tsx handleHomePlanCompleted), NOT the Plan tab\'s broad handlePlanLogged', /setExecutionFacts\(\(current\) => new Map\(current\)\.set\(planId, 'COMPLETED'\)\)/.test(dash) && /await onPlanCompleted\?\.\(\)/.test(dash) && /onPlanCompleted=\{handleHomePlanCompleted\}/.test(strip(read('../apps/web/app/page.tsx'))) && !/onPlanCompleted=\{handlePlanLogged\}/.test(strip(read('../apps/web/app/page.tsx'))));
  check('18/25. reconciliation is secondary: it runs inside its own try/catch AFTER the confirmed overlay and cannot fail the completion (behavior proven in homeRefresh.test.ts)', /setExecutionFacts[\s\S]{0,400}try \{\s*await onPlanCompleted\?\.\(\);\s*\} catch \{/.test(dash));
