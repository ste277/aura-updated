...
async function main() {
  ...
  check('11/15/24. after a CONFIRMED success Home overlays only the confirmed plan id, then runs Home\'s own quiet refresh (page.tsx handleHomePlanCompleted), NOT the Plan tab\'s broad handlePlanLogged', /setLoggedPlanIds\(\(current\) => new Set\(current\)\.add\(planId\)\)/.test(dash) && /await onPlanCompleted\?\.\(\)/.test(dash) && /onPlanCompleted=\{handleHomePlanCompleted\}/.test(strip(read('../apps/web/app/page.tsx'))) && !/onPlanCompleted=\{handlePlanLogged\}/.test(strip(read('../apps/web/app/page.tsx'))));
  check('18/25. reconciliation is secondary: it runs inside its own try/catch AFTER the confirmed overlay and cannot fail the completion (behavior proven in homeRefresh.test.ts)', /setLoggedPlanIds[\s\S]{0,400}try \{\s*await onPlanCompleted\?\.\(\);\s*\} catch \{/.test(dash));
  check('17/26. a failed completion (pre-commit) shows a compact PLAN-OWNED alert, leaves the plan visible and never adds it to the logged set', /if \(result === 'FAILED'\) \{[\s\S]{0,200}setCompleteError\(\{ planId, message: "Couldn't mark that done\. Try again\." \}\);\s*return;\s*\}[\s\S]{0,120}setLoggedPlanIds/.test(dash) && /role="alert"/.test(dash));
