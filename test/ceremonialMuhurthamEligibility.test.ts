...
// ============================================================
// GENERAL/PERSONAL/SHARED IDENTICAL ELIGIBILITY -- all three scopes must
// exclude exactly the same set of dates for the same activity/range, since
// they all funnel through the same evaluateMuhurthamCandidate() with the
// same classification (brief section 39). Personal/shared context is never
// able to rescue or additionally exclude a date on eligibility grounds.
// ============================================================
...
if (personalWide.status === 'OK' && sharedWide.status === 'OK') {
  ...
  check('PERSONAL surfaces exactly the same set of eligible dates as GENERAL (personal factors never rescue or additionally exclude on eligibility)', JSON.stringify(personalDateSet) === JSON.stringify(generalDateSet));
  check('SHARED surfaces exactly the same set of eligible dates as GENERAL (shared factors never rescue or additionally exclude on eligibility)', JSON.stringify(sharedDateSet) === JSON.stringify(generalDateSet));
}
