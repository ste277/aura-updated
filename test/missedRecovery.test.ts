...
async function main() {
  ...
  check('3/4/78. MISSED is derived, not persisted: 38 migrations, no missedAt anywhere in source, no MISSED status written by the plan domain', (() => { const dirs = fs.readdirSync(path.join(__dirname, '../apps/web/prisma/migrations')).filter((d) => /^\d{4}_/.test(d)); const dbSrc = strip(read('../apps/web/lib/db.ts')); const src = [dbSrc, lib, dash, strip(read('../apps/web/lib/planMove.ts'))].join('\n'); return dirs.length === 38 && !/missedAt|missed_at/i.test(src) && !/status\s*=\s*'MISSED'|SET status = 'MISSED'|'MISSED'\s*,\s*'?LOGGED/.test(dbSrc); })());
