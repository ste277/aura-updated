...
check('no migration added by PR B (migrations: 36 + 0037 Skip C1 + 0038 Move D2 + 0039 scheduling mode F1)', fs.readdirSync(path.join(__dirname, '../apps/web/prisma/migrations')).filter((f) => /^\d{4}_/.test(f)).length === 39);
