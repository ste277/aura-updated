...
async function main() {
  ...
  check('3/27. Skip renders only under skippablePlanIdNow (ACTIVE_PLAN), with an activity-specific accessible name, "Skipping…" while in flight, and a real disabled state', /skippablePlanIdNow && \(\s*<TextButton/.test(dash) && /ariaLabel=\{`Skip "\$\{spotlightItem\.title\}"`\}/.test(dash) && /'Skipping…' : 'Skip'/.test(dash) && /disabled=\{skippingPlanIds\.has\(skippablePlanIdNow\) \|\| completingPlanIds\.has\(skippablePlanIdNow\) \|\| movingPlanIds\.has\(skippablePlanIdNow\)( \|\| recompositionAccepting)?\}/.test(dash));
