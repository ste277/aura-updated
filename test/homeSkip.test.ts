...
async function main() {
  ...
  check('3/27. Skip renders only under skippablePlanIdNow (ACTIVE_PLAN), with an activity-specific accessible name, "Skipping…" while in flight, and a real disabled state', /skippablePlanIdNow && \(\s*<TextButton/.test(dash) && /ariaLabel=\{`Skip "\$\{spotlightItem\.title\}"`\}/.test(dash) && /'Skipping…' : 'Skip'/.test(dash) && /disabled=\{skippingPlanIds\.has\(skippablePlanIdNow\) \|\| completingPlanIds\.has\(skippablePlanIdNow\)\}/.test(dash));
  check('3. the whole Right Now card is not a Skip control: one call site, the Skip button itself', (dash.match(/handleSkipRightNow\(/g) ?? []).length === 1 && /onClick=\{\(\) => void handleSkipRightNow\(skippablePlanIdNow\)\}/.test(dash));
  ...
  check('7/8. one execution guard: both handlers consult the same executor (isBusy) before touching state, and there is no second guard', (dash.match(/planExecutor\.current\.isBusy\(planId\)/g) ?? []).length === 2 && !/planCompleter|planSkipper/.test(dash) && (helper.match(/const inFlight = new Set/g) ?? []).length === 1);
  ...
  check('9/11/14. one execution-fact map feeds the timeline overlay, the reminder filter and the next-item filter', /useState<ReadonlyMap<string, ExecutionOutcome>>/.test(dash) && /overlayExecutionFacts\(composedTimeline, executionFacts\)/.test(dash) && /selectVisibleStartingSoonReminder\(startingSoonReminders, executionFacts\)/.test(dash) && /agendaWithoutResolvedNext\(myDayAgenda, executionFacts\)/.test(dash));
  check('40/41/42. no confirmation modal, no replan/Move/carry-forward CTA and no Day Constructor call in the Skip path', !/ModalShell|Move|Reschedule|Carry|Plan again|orchestrateConstructDay|constructDay/.test(dash.slice(dash.indexOf('const handleSkipRightNow'), dash.indexOf('const spotlightExplanation'))) && !/Plan again|Carry forward/.test(dash));
  ...
  check('64. no migration by C2 (37 total)', fs.readdirSync(path.join(__dirname, '../apps/web/prisma/migrations')).filter((f) => /^\d{4}_/.test(f)).length === 37);
