...
async function main() {
  ...
  check('scope: no schema/migration change, no lifecycle/planning/acceptance file references Home capture', fs.readdirSync(path.join(__dirname, '../apps/web/prisma/migrations')).filter((f) => /^\d{4}_/.test(f)).length === 37 && !/quickCapture|HomeQuickCapture/i.test(strip(read('../apps/web/lib/planDayEntry.ts')) + strip(read('../apps/web/lib/dayConstructorAcceptancePersistence.ts')) + strip(read('../apps/web/lib/captures.ts'))));
