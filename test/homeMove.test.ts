...
async function main() {
  ...
  check('29. after success focus goes to the successor row when it is on today\'s list, else the stable Right Now region (no keyboard-raising input focus)', /focusMoveSuccessor\(successor\.id\)/.test(handler) && /data-timeline-item-id="plan:\$\{successorId\}"[\s\S]{0,260}data-home-right-now-label/.test(dash) && /data-timeline-item-id=\{item\.id\}\s+tabIndex=\{-1\}/.test(strip(read('../apps/web/components/HomeTimeline.tsx'))));
