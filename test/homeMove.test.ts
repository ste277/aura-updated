...
async function main() {
  ...
  check('36/37. timezone conversion goes through the canonical utilities (localDateTimeToUTC/getDatePartsInTimezone/addDaysToDateStr) and no new timezone math exists', /from '\.\/timezone'/.test(helper) && /localDateTimeToUTC\(moveDayDate/.test(helper) && !/Intl\.DateTimeFormat|getTimezoneOffset|toLocaleString\('en-US', \{ timeZone/.test(helper.replace(/toLocaleTimeString\('en-US', \{ timeZone: timezone/g, '')));
