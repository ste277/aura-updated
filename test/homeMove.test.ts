...
import { defaultMoveSelection, resolveMoveDestination, moveDayDate, parseMoveResponse, moveFailureMessage, moveDestinationMessage, applyConfirmedSuccessors, hideMovedTimelineItems, formatMoveTime } from '../apps/web/lib/homeMove';
...
async function main() {
  ...
  check('9. the default is a non-committed sensible time: next quarter hour at least 30 min ahead (11:07 -> 11:45), Today', dflt.day === 'TODAY' && dflt.time === '11:45');
  const lateDefault = defaultMoveSelection(at('23:50'), TZ);
  check('9. late in the day the default rolls to Tomorrow (23:50 -> 00:30 tomorrow), never a past time', lateDefault.day === 'TOMORROW' && lateDefault.time === '00:30');
  check('9. the default never resolves to a past instant', (() => { const r = resolveMoveDestination(dflt, A.plannedStartAt.toISOString(), at('11:07'), TZ); return r.ok && Date.parse(r.newStartAt) > at('11:07').getTime(); })());
  ...
  check('21. the default never lands on a repeated wall time: NY fall-back night (now 00:40 EDT) skips the ambiguous 01:xx hour to the next unique time (02:00 EST); an ordinary time is unchanged', dfAmb.day === 'TODAY' && dfAmb.time === '02:00' && dfOrdinary.day === 'TODAY' && dfOrdinary.time === '04:45' && resolveMoveDestination(dfAmb, cur0, new Date('2026-11-01T04:40:00Z'), 'America/New_York').ok);
  check('21. the default is always a wall time that resolves (checked across a transition-day sweep, every 15 minutes)', (() => { for (const [tz, start] of [['America/New_York', '2026-03-08T00:00:00Z'], ['America/New_York', '2026-11-01T00:00:00Z'], ['America/Los_Angeles', '2026-11-01T05:00:00Z'], ['Europe/London', '2026-10-25T00:00:00Z'], ['Australia/Sydney', '2026-04-04T13:00:00Z']] as const) for (let k = 0; k < 96; k++) { const now = new Date(Date.parse(start) + k * 15 * 60000); const sel = defaultMoveSelection(now, tz); const r = resolveMoveDestination(sel, cur0, now, tz); if (!r.ok) return false; } return true; })());
  ...
  check('4/5. Move renders only under moveablePlanIdNow (ACTIVE or IMMINENT), with an activity-specific accessible name and a real disabled state', /moveablePlanIdNow && \(\s*<TextButton/.test(dash) && /ariaLabel=\{`Move "\$\{spotlightItem\.title\}"`\}/.test(dash) && /disabled=\{movingPlanIds\.has\(moveablePlanIdNow\) \|\| completingPlanIds\.has\(moveablePlanIdNow\) \|\| skippingPlanIds\.has\(moveablePlanIdNow\)\}/.test(dash));
  ...
  check('6/8/33. the picker is an accessible form: labelled Day select and time input (htmlFor/id pairs), type="time" step=60 (no seconds), a submit button, a Cancel button, a form label naming the activity', /<FieldLabel htmlFor="home-move-day">Day<\/FieldLabel>/.test(dash) && /id="home-move-day"/.test(dash) && /<FieldLabel htmlFor="home-move-time">Start time<\/FieldLabel>/.test(dash) && /id="home-move-time" type="time" step=\{60\}/.test(dash) && /<PrimaryButton type="submit"/.test(dash) && /aria-label=\{`Move "\$\{spotlightItem\.title\}" to another time`\}/.test(dash) && !/duration.*<TextInput|type="number"/.test(dash.slice(dash.indexOf('home-move-day'), dash.indexOf('home-move-day') + 1500)));
  ...
  check('32. Enter/confirm is a single form submit routed to the guarded handler', /onSubmit=\{\(event\) => \{\s*event\.preventDefault\(\);\s*void handleMoveRightNow\(moveablePlanIdNow, spotlightItem\.start\);/.test(dash) && (dash.match(/handleMoveRightNow\(/g) ?? []).length === 1);
  ...
  check('29. after success focus goes to the successor row when it is on today\'s list, else the stable Right Now region (no keyboard-raising input focus)', /data-timeline-item-id="plan:\$\{successor\.id\}"[\s\S]{0,120}data-home-right-now-label/.test(handler) && /data-timeline-item-id=\{item\.id\}\s+tabIndex=\{-1\}/.test(strip(read('../apps/web/components/HomeTimeline.tsx'))));
  ...
  check('43/62. MISSED exposure, new lifecycle state, schema and migrations are absent: 38 migrations, ExecutionOutcome adds only the confirmed MOVED fact', fs.readdirSync(path.join(__dirname, '../apps/web/prisma/migrations')).filter((f) => /^\d{4}_/.test(f)).length === 38 && /export type ExecutionOutcome = 'COMPLETED' \| 'SKIPPED' \| 'MOVED';/.test(exec));
  check('22. presentation choice: the live Timeline hides MOVED rows via one helper; the read model (agenda, DB) keeps them', /hideMovedTimelineItems\(overlayExecutionFacts\(composedTimeline, executionFacts\)\)/.test(dash));
