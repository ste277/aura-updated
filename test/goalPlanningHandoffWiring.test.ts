...
function main() {
  ...
  check('39. 0035_goals is present and no handoff/goal-link migration was added by PR C', migrationDirs.includes('0035_goals') && !migrationDirs.some((d) => /handoff|goal.?link|goalactivity.?link/i.test(d)) && migrationDirs.filter((d) => d > '0035_goals').every((d) => d === '0036_captures' || d === '0037_planned_activity_skipped_at'));
