...
check('13/14. the generic link helpers only widened their replaceable set to include a stale MOVED link (unchanged otherwise)', (db.match(/status IN \('CANCELLED', 'SKIPPED', 'MOVED'\)/g) ?? []).length === 2);
const comp = strip(read('../apps/web/components/PlanWithAuraView.tsx')) + strip(read('../apps/web/components/HomeDashboard.tsx'));
check("33/72. scope: no UI calls the move endpoint (no Home button, no time picker), and the Plan tab's existing reschedule workflow (save new + DELETE old) is untouched", !/\/move\b/.test(comp) && /fetch\(`\/api\/plans\/\$\{replacedPlanId\}`, \{ method: 'DELETE' \}\)/.test(comp));
check('72. scope: no Home Move overlay in the Home execution model (ExecutionOutcome stays COMPLETED | SKIPPED)', /export type ExecutionOutcome = 'COMPLETED' \| 'SKIPPED';/.test(strip(read('../apps/web/lib/homeCompletion.ts'))));
